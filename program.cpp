...
void program::parse_line(int line_num){
...
int program::string_convert(std::string& input_line){
    //判断语句是否有行号
    ...
    if(have_line_num){ //有行号的输入
        ...
        if(state_op >= 1 && state_op <= 3){ //let/print/input

        }else if(state_op >= 8){ //RUN/LOAD/LIST/CLAER/HELP/QUIT
...
void program::store_line_number(int line_num){
...
QString program::print_line(int line_num){
...
void program::store_expreesions(int line_num){
...
Statement program::get_state_op(int &line_index){
...
void program::run_program(int &line_index){
...
void program::run_let(int &line_index){
...
void program::run_print(int &line_index){
    int cur_line_num = line_numbers[line_index];
...
void program::run_goto(int &line_index){
    int cur_line_num = line_numbers[line_index];
...
void program::run_if(int &line_index){
    int cur_line_num = line_numbers[line_index];
...
void program::run_input(int &line_index){
    int cur_line_num = line_numbers[line_index];
...
Cmp program::string_to_cmp(std::string &str){
    if(str == "="){
        return Equal;
    }else if(str == ">"){
        return Greater;
...
bool program::is_true(int num1, int num2, Cmp op){
    switch (op) {
    case Equal:{
        return num1 == num2;
    }
...
int program::find_line_num_index(int &line_num){
    int left = 0, right = line_numbers.size();
    while(left < right){
        int mid = (right - left) / 2 + left;
